...
namespace eSolutionTech.Manager.Controllers
{
    public class TimeOffTypeController : Controller
    {
        ...
        {
            return View(new TimeOffTypeDeleteRequest()
            {
                Id = id
            });
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Delete(TimeOffTypeDeleteRequest request)
        {
            if (!ModelState.IsValid)
                return View();

            var result = await _timeOffTypeApiClient.DeleteTimeOffType(request.Id);
            if (result)
            {
